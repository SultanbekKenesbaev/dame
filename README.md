...
- Привязка персонального ПК прямо в Windows-приложении по 15-минутному коду и ECDSA-ключу устройства.
...
installer/                        WiX MSI, автозапуск и дополнительные kiosk-инструменты
...
Обычному пользователю нужен только готовый MSI. После установки служба запускается автоматически, открывается DailyGate и показывает экран подключения устройства. Администратор создаёт в веб-панели 15-минутный код, пользователь вводит его прямо в программе, после чего появляется вход сотрудника по логину и паролю. PowerShell и ручная настройка для обычного режима не нужны.

DailyGate добавляется в меню «Пуск» и автоматически запускается при каждом входе в Windows. В обычном режиме рабочий стол остаётся доступным, а окно можно свернуть, развернуть или закрыть. Полный kiosk на Enterprise/Education/IoT сохраняется как отдельная техническая возможность и настраивается вручную через `Provision-Device.ps1 -KioskMode`. Перед первым пилотом прочитайте [инструкцию пилота](docs/WINDOWS-PILOT.md).
