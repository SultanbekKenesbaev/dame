...
- Windows Service, полноэкранный WPF-клиент, несколько мониторов и принудительный logoff в 04:00.
- Shell Launcher provisioning, auto-logon kiosk profile и recovery через отдельного Windows-администратора.
...
```powershell
& "$env:ProgramFiles\DailyGate\Tools\Provision-Device.ps1" `
  -ApiUrl "https://dailygate.example.com" `
  -EnrollmentCode "КОД-ИЗ-АДМИНКИ" `
  -DeviceName "Reception-01"
```

Shell Launcher требует Windows Enterprise, Education или IoT Enterprise. Перед первым пилотом обязательно прочитайте [инструкцию пилота](docs/WINDOWS-PILOT.md).
